...
This guide uses AWS S3, but the same upload flow works with every supported provider.

To use ImageKit or a custom provider, replace the provider inside the storage profile. Your asset rules, `prepareUpload()` call, and browser upload flow can remain the same.
